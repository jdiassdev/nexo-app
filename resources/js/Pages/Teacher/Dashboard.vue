    ...
    <AppLayout title="Minhas Disciplinas" :nav-items="navItems">
        ...
        <div v-if="subjects.length === 0" class="page-card">
            <div class="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p class="text-sm font-medium text-slate-600">Nenhuma disciplina atribuída</p>
            ...
            >
                ...
                <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center shrink-0">
                            <i class="pi pi-book text-indigo-600 text-sm" />
                        </div>
                        <div>
                            <h2 class="font-semibold text-slate-800 leading-tight">{{ subject.name }}</h2>
                ...
                <div class="grid grid-cols-4 divide-x divide-slate-100">
                    <div class="px-5 py-3.5">
                        ...
                        <p class="text-xl font-bold text-slate-700">{{ subject.activities_count }}</p>
                    </div>
                    <div class="px-5 py-3.5">
                        <p class="text-xs text-slate-400 mb-1">Provas</p>
                        <p class="text-xl font-bold text-slate-700">{{ subject.exams_count }}</p>
                    </div>
                    <div class="px-5 py-3.5">
                        <p class="text-xs text-slate-400 mb-1">Média da turma</p>
                        <p class="text-xl font-bold"
                            :class="subject.avg_score === null ? 'text-slate-300' : subject.avg_score >= passingGrade ? 'text-emerald-600' : 'text-red-500'"
                        >
