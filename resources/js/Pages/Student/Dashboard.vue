    ...
    <AppLayout title="Minhas Disciplinas" :nav-items="navItems">
        ...
        <div v-if="subjects.length === 0" class="page-card">
            <div class="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p class="text-sm font-medium text-slate-600 dark:text-slate-300">Nenhuma disciplina encontrada</p>
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <div
                ...
                :class="subject.final !== null && !subject.passing ? 'ring-1 ring-red-200 dark:ring-red-900' : ''"
            >
                <!-- Header -->
                <div class="px-5 pt-5 pb-4 border-b border-slate-100 dark:border-slate-800">
                    <div class="flex items-start justify-between gap-2 mb-3">
                        <div class="min-w-0">
                            <h2 class="font-semibold text-slate-800 dark:text-slate-100 leading-tight truncate">{{ subject.name }}</h2>
                        ...
                        <span
                            ...
                            :class="subject.passing ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' : 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400'"
                        >
                    ...
                    <div v-if="subject.schedules.length" class="flex flex-wrap gap-1">
                        <span
                            ...
                            :key="i"
                            class="inline-flex items-center gap-1 text-xs bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded-full"
                        >
                ...
                <!-- Stats -->
                <div class="px-5 py-4 flex gap-6 border-b border-slate-100 dark:border-slate-800">
                    <div>
                        <p class="text-xs text-slate-400 mb-0.5">Média Final</p>
                        <p class="text-2xl font-bold"
                            :class="subject.final === null ? 'text-slate-300 dark:text-slate-600' : subject.passing ? 'text-emerald-600' : 'text-red-500'">
                            {{ subject.final ?? '—' }}
                        </p>
                    </div>
                    <div>
